...
const AllUsers: FC<AllUsersProps> = (props) => {
  ...
  const [updateUserRole, { isSuccess: isUpdated, isError: updateError, error: updateErrorData }] =
    useUpdateUserRoleMutation();

  const [deleteUserAdminRole, { isSuccess: isRoleDeleted, isError: deleteRoleError, error: deleteRoleErrorData }] =
    useDeleteUserAdminRoleMutation();

  const [deleteUser, { isSuccess: isDeleted, isError: deleteError, error: deleteErrorData }] = useDeleteUserMutation();

  const [blockUser, { isSuccess: isBlocked, isError: blockError, error: blockErrorData }] = useBlockUserMutation();

  const [unLockUser, { isSuccess: isUnLocked, isError: unLockError, error: unLockErrorData }] = useUnLockUserMutation();
  ...
  if (Array.isArray(usersData)) {
    if (isTeam && Array.isArray(newUsersData)) {
      ...
      usersData?.forEach((user: any) => {
        const createdAt = new Date(user?.createdAt || '');
        const formattedDate = !isNaN(createdAt.getTime()) ? createdAt.toLocaleDateString('en-US') : 'Ngày không hợp lệ';
        rows.push({
          id: user?._id,
          name: user?.name,
          email: user?.email,
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params: any) => (
        <Box className={`mt-2 flex ${isTeam ? 'flex justify-center' : 'items-center justify-center'} `}>
          {!isTeam &&
            (params.row.isBlocked === 'Có' ? (
              ...
              >
                <IoIosUnlock size={20} className="text-black dark:text-white" />
              </Button>
            ) : (
              <Button
              ...
              >
                <IoIosLock size={20} className="text-black dark:text-white" />
              </Button>
            ))}
          {isTeam ? (
            <Button
            ...
            >
              <AiOutlineDelete size={20} className="w-full text-black dark:text-white" />
            </Button>
          ) : (
            <Button
